...
func get_available_tools() -> Array:
	...
	# NOTE:可见工具列表已在服务端修改；未来如果更新 godot-mcp-server 或 MCP 客户端后
	# 出现工具调用异常（如 Unknown tool / 参数不匹配），请优先清掉缓存再试。
	# 缓存路径：C:\Users\MLTZ\AppData\Local\npm-cache\_npx\4776078e0257f836\node_modules\godot-mcp-server
	var allowed: Array[String] = [] # 允许暴露的工具白名单，空数组表示全部暴露
	allowed = [
		'map_project',
		'map_scenes',
		'run_scene',
	...
	return _tool_map.keys().filter(func(k): return k in allowed)
