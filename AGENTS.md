...
- **框选与复制粘贴**：框选建筑（蓝色高亮），Ctrl+C/X/V 复制/剪切/粘贴，Ctrl+Z 撤销，Ctrl+Shift+Z 或 Ctrl+Y 重做
- **快捷键提示面板**：KeyHints 在游戏界面右上角动态显示当前可用快捷键及模式说明（放置/删除/框选/粘贴/吸取），随当前模式变化实时更新
- **吸管功能**：鼠标滚轮在已有建筑上按下时，自动切换为该建筑对应的建造类型，无需手动选择
- **流体系统**：FluidCoordinator 通过脏标记（_dirty）缓存 BFS 连通网络结构，仅在建筑放置/删除时重建网络拓扑（监听 building_placed/building_removed 事件）；每 tick 遍历缓存的网络执行水量分配（水源产出汇总 → 均分给所有有空位的容器）；管道作为纯导体不存储水，视觉三档反映网络状态（0=未连通/1=输送中/2=满载）
