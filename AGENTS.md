...
- 修改后运行测试：`& "C:\Users\MLTZ\Desktop\Godot_v4.6.1-stable_win64.exe" --headless '--path' 'C:\Users\MLTZ\Desktop\程序\godot\bili游戏大赛\demo' '--script' 'res://addons/gut/gut_cmdln.gd'`，结果导出至 `save/test_results.xml`（`disable_colors` 和 `junit_xml_file` 已在 `.gutconfig.json` 中配置，无需命令行指定）
- **验证测试结果无需查看完整输出**：检查命令退出码（`exit_code == 0` = 全部通过）和 `save/test_results.xml` 的 `failures` 属性（`failures="0"` = 全部通过）。全部通过时 XML 文件仅含空头部（`tests="0"` 表示无测试用例写入，因为配置为仅输出未通过测试），无需重定向或查看终端完整输出
