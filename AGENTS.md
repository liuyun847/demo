...
│   │   ├── test_fps_display.gd # FPSDisplay 帧率显示测试
...
- **单元测试**：基于GUT框架，覆盖BuildingData、GridCoordinate、流体节点、FPSDisplay、BuildingManager、SelectionManager、SaveManager、FluidCoordinator、KeybindManager、CameraController、InfiniteGridMap、InventoryBar、BuildingTooltip、GameConfig等核心模块；在Godot编辑器底部GUT面板点击"Run All"运行，或通过命令行运行（注意：PowerShell 中需使用单引号包裹 `--` 开头的参数，避免被解析为运算符；加 `--headless` 和 `-gdisable_colors` 避免输出截断，加 `-gjunit_xml_file=res://save/test_results.xml` 导出完整结果）

# 开发规范

- 对于不确定的接口，先查询，禁止猜测用法
- 修改后使用godot-debug技能或mcp工具进行代码静态检查和运行时错误检测，确保无错误
- 修改后运行相关测试：`& "C:\Users\MLTZ\Desktop\Godot_v4.6.1-stable_win64.exe" --headless '--path' 'C:\Users\MLTZ\Desktop\程序\godot\bili游戏大赛\demo' '--script' 'res://addons/gut/gut_cmdln.gd' '-gdir=res://tests/unit' '-gdir=res://tests/integration' '-ginclude_subdirs' '-gexit' '-gdisable_colors' '-gjunit_xml_file=res://save/test_results.xml'`，结果导出至项目 `save/test_results.xml`
