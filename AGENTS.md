...
│   │   ├── reaction_recipe.gd   # 反应配方数据（Resource）
...
│   ├── reaction/               # 反应/连通网络系统模块
│   │   ├── reaction_coordinator.gd # 反应协调器（BFS 网络 + 元素扩散+反应驱动）
│   │   ├── element_grid.gd        # 世界格子元素管理器（ElementGrid）
│   │   ├── element_diffusion.gd   # 元素扩散系统（重力/横向扩散）
│   │   ├── element_reaction.gd    # 接触反应系统（相邻格子反应检测）
...
│   ├── emitter_fire_icon.svg  # 火喷口建筑槽位图标
│   ├── emitter_earth_icon.svg # 土喷口建筑槽位图标
...
| ElementRegistry | res://scripts/elements/element_registry.gd | 元素类型注册表与反应配方查询 |
...
| reaction\_occurred         | grid\_pos: Vector2i, reactant\_a\_id: String, reactant\_b\_id: String, product\_id: String | 反应发生时 |
...
- **建筑类型选择**：底部 10 槽位，数字键 1~0 切换，选中槽位高亮；重复按下已选中槽位可取消选择；1 号=容器(ContainerNode)，2 号=管道(PipeNode)，3 号=水喷口(EmitterNode-water)，4 号=砖块(BrickNode)，5 号=火喷口(EmitterNode-fire)，6 号=土喷口(EmitterNode-earth)，7 号=收集器(CollectorNode)；未解锁类型显示锁定状态
...
- **发射器建筑（EmitterNode）**：A 型建筑，消耗源质（essence\_cost\_per\_tick）向 output\_direction 方向生成指定元素类型的元素；支持水/火/土三种类型（对应水喷口/火喷口/土喷口），默认方向由元素重力方向决定
...
- **反应/连通检测系统**：ReactionCoordinator 通过脏标记（\_dirty）缓存 BFS 连通网络结构，仅在建筑放置/删除时重建网络拓扑；每 tick 先执行 BFS 网络检测，然后依次调用元素扩散（ElementDiffusion）和接触反应（ElementReaction）
- **元素系统**：5 种元素类型（水/火/土/岩浆/岩石），2 条线性配方（火+土→岩浆，水+岩浆→岩石）；元素通过 ElementGrid 管理格子占用（与建筑互斥），ElementDiffusion 处理重力驱动的扩散（水下沉、火上浮、土和岩石不移动），ElementReaction 检测相邻格子的反应；ElementRegistry（Autoload）提供元素类型查询和反应配方查找；ElementRenderer 响应 element\_spawned/element\_removed 信号，通过 \_draw() 渲染彩色方块
- **源质阈值进度系统（ProgressSystem）**：Autoload 单例，监听 EssencePool.essence\_changed 信号，达到预设阈值时触发 essence\_threshold\_reached 信号（同时通过 EventBus 转发），解锁新建筑类型；阈值阶梯：0→100→500→2000→5000→10000→50000
- **源质数量显示（EssenceDisplay）**：右上角 UI 组件（游戏开始后由 main.gd 动态创建），监听 EssencePool.essence\_changed 实时显示当前源质余额（金色字体）
...
3. 操作说明（默认按键，可在设置页面修改）：
   ...
   - 鼠标右键单击：删除单个建筑
   - 鼠标右键拖拽：矩形框选批量删除建筑
   - 数字键 1~0：切换建筑类型（对应底部选择栏 10 个槽位）；重复按下已选中槽位可取消选择；1=容器，2=管道，3=水喷口，4=砖块，5=火喷口，6=土喷口，7=收集器
   - ESC 键：显示/关闭开始菜单
   - Ctrl+C：复制选中的建筑
   - Ctrl+X：剪切选中的建筑
   - Ctrl+V：进入粘贴模式
