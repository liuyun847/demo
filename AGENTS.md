...
```
GameConfig（无依赖） → EventBus（无依赖） → ElementRegistry（无依赖） → KeybindManager（依赖 GameConfig） → SelectionManager（运行时获取 BuildingManager）
```
...
- **元素系统**：5 种元素类型（水/火/土/岩浆/岩石），2 条线性配方（火+土→岩浆，水+岩浆→岩石）；元素通过 ElementGrid 管理格子占用（与建筑互斥），ElementDiffusion 处理重力驱动的扩散（水下沉、火上浮、土和岩石不移动），ElementReaction 检测相邻格子的反应；ElementRegistry（Autoload）提供元素类型查询和反应配方查找
