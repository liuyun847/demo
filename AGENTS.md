...
│   └── buildings/             # 建筑类型贴图（类型3~10）
│       ├── building_01.svg ~ building_10.svg  # 10种建筑贴图（01/02旧版未使用）
├── scenes/                    # 场景文件目录
│   ├── main.tscn              # 游戏主场景文件
...
| 单例名称         | 脚本路径                                      | 用途                |
| ------------ | ----------------------------------------- | ----------------- |
| GameConfig     | res\://scripts/autoload/game\_config.gd     | 游戏配置与常量集中管理（含 container_type_id, pipe_type_id, SAVE_VERSION 等）       |
...
- **建筑类型选择**：底部10槽位，数字键1~0切换，选中槽位高亮；1号=容器(ContainerNode)，2号=管道(PipeNode)
...
- **流体系统**：基于压力梯度的分布式流体算法，每个流体节点（水管/容器/水源）独立计算压力差，通过两步提交模式向低压力邻居推水，无需中心调度器即可自然实现均分、防环、防溢出
