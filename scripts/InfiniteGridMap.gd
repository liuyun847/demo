...
func _draw() -> void:
	...
	var adjusted_thin_width = GameConfig.thin_line_width / current_zoom
	var adjusted_thick_width = GameConfig.thick_line_width / current_zoom
	...
	for block_coord in loaded_blocks.keys():
		...
		if show_thin_lines:
			for x in range(1, GameConfig.big_cell_size):
				var line_x = block_offset.x + x * GameConfig.cell_size
