...
func test_ghost_show_and_hide() -> void:
	...
	assert_eq(_gpm.get_layer_cells("ghost").size(), 2, "ghost 层应有 2 个格子")
	_gpm.hide_ghost()
...
func test_remove_ghost_show_and_hide() -> void:
	...
	assert_eq(_gpm.get_layer_cells("remove_ghost").size(), 2)
	_gpm.hide_remove_ghost()
	assert_true(_gpm.get_layer_cells("remove_ghost").is_empty())

func test_set_selected_cells() -> void:
	var cells: Array[Vector2i] = [Vector2i(1, 1), Vector2i(2, 2)]
	_gpm.set_selected_cells(cells)
	assert_eq(_gpm.get_layer_cells("selected").size(), 2, "selected 层应有 2 个格子")
	assert_eq(_gpm.get_layer_cells("selected")[0], Vector2i(1, 1))
	var empty: Array[Vector2i] = []
	_gpm.set_selected_cells(empty)
	assert_true(_gpm.get_layer_cells("selected").is_empty(), "空数组应清空 selected 层")
...
func test_set_paste_preview_line_single_anchor() -> void:
	...
	assert_eq(_gpm.get_layer_cells("paste_ghost").size(), 2, "单锚点应计算 2 个粘贴预览格子")
...
func test_set_paste_preview_line_dedup() -> void:
	...
	_gpm.set_paste_preview_line(anchors, clipboard)
	assert_eq(_gpm.get_layer_cells("paste_ghost").size(), 1, "重复锚点应去重，只有 1 个预览格子")
...
func test_clear_paste_preview() -> void:
	...
	_gpm.set_paste_preview_line(anchors, clipboard)
	assert_false(_gpm.get_layer_cells("paste_ghost").is_empty(), "设置后应有预览")
	_gpm.clear_paste_preview()
	assert_true(_gpm.get_layer_cells("paste_ghost").is_empty(), "清除后 paste_ghost 层应为空")
...
func test_select_ghost_show_and_hide() -> void:
	...
	assert_eq(_gpm.get_layer_cells("select_ghost").size(), 3, "应有 3 个选择幽灵格子")
	_gpm.hide_select_ghost()
	assert_true(_gpm.get_layer_cells("select_ghost").is_empty(), "隐藏后 select_ghost 层应为空")

func test_deselect_ghost_show_and_hide() -> void:
	var cells: Array[Vector2i] = [Vector2i(3, 3)]
	_gpm.show_deselect_ghost(cells)
	assert_eq(_gpm.get_layer_cells("deselect_ghost").size(), 1, "应有 1 个取消选择幽灵格子")
	_gpm.hide_deselect_ghost()
	assert_true(_gpm.get_layer_cells("deselect_ghost").is_empty(), "隐藏后 deselect_ghost 层应为空")

func test_set_paste_preview_line_empty_clipboard() -> void:
	var anchors: Array[Vector2i] = [Vector2i(0, 0)]
	_gpm.set_paste_preview_line(anchors, {})
	assert_true(_gpm.get_layer_cells("paste_ghost").is_empty(), "空剪贴板不应有行预览")
