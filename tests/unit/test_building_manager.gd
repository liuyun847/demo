...
func test_place_building_returns_false_when_occupied() -> void:
	...
	assert_false(_bm.place_building(Vector2i(3, 3), GameConfig.PIPE_TYPE_ID))

func test_place_building_pipe_type_2() -> void:
	var result: bool = _bm.place_building(Vector2i(1, 2), GameConfig.PIPE_TYPE_ID)
	assert_true(result)
...
func test_get_all_buildings_data() -> void:
	...
	assert_eq(data.size(), 6, "核心 4 格 + 2 个建筑 = 6 个记录")
	assert_true(data.has(Vector2i(0, 0)), "应包含核心 (0, 0)")
	assert_true(data.has(Vector2i(5, 5)), "应包含 (5, 5)")
	assert_true(data.has(Vector2i(6, 5)), "应包含 (6, 5)")
...
func test_clear_all_buildings() -> void:
	_bm.place_building(Vector2i(5, 5), GameConfig.PIPE_TYPE_ID)
	_bm.place_building(Vector2i(6, 6), GameConfig.PIPE_TYPE_ID)
	_bm.clear_all_buildings()
	assert_eq(_bm.get_all_buildings_data().size(), 4, "清除后应只剩核心的 4 个格子")
...
func test_place_buildings_in_line() -> void:
	...
	for i in range(5):
		assert_true(_bm.has_building(Vector2i(i + 2, 0)))

func test_remove_buildings_in_rect() -> void:
	for x: int in range(3):
		for y: int in range(3):
			_bm.place_building(Vector2i(x + 5, y + 5), GameConfig.PIPE_TYPE_ID)
...
func test_get_building_type() -> void:
	assert_eq(_bm.get_building_type(Vector2i(0, 0)), GameConfig.CORE_TYPE_ID, "(0,0) 应为核心类型")
	_bm.place_building(Vector2i(5, 5), GameConfig.PIPE_TYPE_ID)
	assert_eq(_bm.get_building_type(Vector2i(5, 5)), GameConfig.PIPE_TYPE_ID, "应返回正确的建筑类型")
	assert_eq(_bm.get_building_type(Vector2i(99, 99)), "", "不存在的位置应返回空字符串")

func test_get_building_data() -> void:
	var core_data: BuildingData = _bm.get_building_data(Vector2i(0, 0))
	assert_not_null(core_data, "(0,0) 应有核心数据")
	if core_data:
		assert_eq(core_data.building_type, GameConfig.CORE_TYPE_ID, "核心类型应正确")
	_bm.place_building(Vector2i(5, 5), GameConfig.PIPE_TYPE_ID)
	var data: BuildingData = _bm.get_building_data(Vector2i(5, 5))
	assert_not_null(data, "存在的位置应返回 BuildingData")
	if data:
		assert_eq(data.building_type, GameConfig.PIPE_TYPE_ID, "building_type 应正确")
...
func test_get_building_node() -> void:
	...
	assert_true(node is PipeNode, "应为 PipeNode 类型")
...
func test_refresh_pipe_connections_on_place() -> void:
	_bm.place_building(Vector2i(5, 5), GameConfig.PIPE_TYPE_ID)
	var pipe: PipeNode = _bm.get_building_node(Vector2i(5, 5)) as PipeNode
	assert_eq(pipe.connection_mask, 0, "孤立管道连接掩码应为 0")
	_bm.place_building(Vector2i(5, 6), GameConfig.PIPE_TYPE_ID)
	var pipe_adj: PipeNode = _bm.get_building_node(Vector2i(5, 5)) as PipeNode
	assert_true(pipe_adj.connection_mask > 0, "相邻管道放置后连接掩码应更新")
...
func test_refresh_pipe_connections_on_remove() -> void:
	...
	_bm.remove_building(Vector2i(5, 6))
	assert_eq(pipe.connection_mask, 0, "移除邻居后连接掩码应清零")


## 测试13: clear_all_buildings 清除非核心建筑时，building_removed 信号对每个非核心建筑触发一次
func test_clear_all_buildings_emits_signal_per_building() -> void:
	# 放置 3 个非核心建筑
	_bm.place_building(Vector2i(5, 5), GameConfig.PIPE_TYPE_ID)
	_bm.place_building(Vector2i(6, 6), GameConfig.PIPE_TYPE_ID)
	_bm.place_building(Vector2i(7, 7), GameConfig.PIPE_TYPE_ID)
	# 监听 EventBus 的 building_removed 信号
	watch_signals(EventBus)
	_bm.clear_all_buildings()
	# 应触发 3 次 building_removed 信号（每个非核心建筑一次）
	assert_signal_emit_count(EventBus, "building_removed", 3, "应触发 3 次 building_removed 信号")
