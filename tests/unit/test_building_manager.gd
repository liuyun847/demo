...
func test_get_line_cells_horizontal():
	var cells = BuildingManager.get_line_cells(Vector2i(0, 5), Vector2i(4, 5))
...
func test_get_line_cells_vertical():
	var cells = BuildingManager.get_line_cells(Vector2i(3, 0), Vector2i(3, 3))
...
func test_get_line_cells_reverse():
	var cells = BuildingManager.get_line_cells(Vector2i(4, 5), Vector2i(0, 5))
	assert_eq(cells.size(), 5, "反向水平线也应有 5 个格子")

func test_get_line_cells_single_point():
	var cells = BuildingManager.get_line_cells(Vector2i(2, 2), Vector2i(2, 2))
	assert_eq(cells.size(), 1, "单点应返回 1 个格子")
	assert_eq(cells[0], Vector2i(2, 2))

func test_get_rect_cells():
	var cells = BuildingManager.get_rect_cells(Vector2i(1, 1), Vector2i(3, 3))
	assert_eq(cells.size(), 9, "3x3 矩形应有 9 个格子")

func test_get_rect_cells_single():
	var cells = BuildingManager.get_rect_cells(Vector2i(5, 5), Vector2i(5, 5))
	assert_eq(cells.size(), 1, "单点矩形应返回 1 个格子")

func test_get_rect_cells_reverse():
	var cells = BuildingManager.get_rect_cells(Vector2i(3, 3), Vector2i(1, 1))
	assert_eq(cells.size(), 9, "反向矩形也应有 9 个格子")

func test_place_buildings_in_line():
	var cells = BuildingManager.get_line_cells(Vector2i(0, 0), Vector2i(4, 0))
...
func test_remove_buildings_in_rect():
	...
	var cells = BuildingManager.get_rect_cells(Vector2i(0, 0), Vector2i(2, 2))
	var removed = _bm.remove_buildings_in_rect(cells)
	assert_eq(removed, 9, "应成功删除 9 个建筑")

func test_ghost_show_and_hide():
	_bm.show_ghost([Vector2i(0, 0), Vector2i(1, 1)])
...
func test_remove_ghost_show_and_hide():
	_bm.show_remove_ghost([Vector2i(2, 2), Vector2i(3, 3)])
...
func test_get_buildings_in_cells():
	...
	var result = _bm.get_buildings_in_cells([Vector2i(0, 0), Vector2i(0, 1), Vector2i(0, 2)])
