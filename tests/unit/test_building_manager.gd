...
func test_bulk_clear_marks_fluid_coordinator_dirty() -> void:
	_bm.place_building(Vector2i(0, 0), GameConfig.pipe_type_id)
	var coordinator = _bm.get_node_or_null("FluidCoordinator")
	if coordinator:
...
func test_refresh_pipe_connections_on_place() -> void:
	_bm.place_building(Vector2i(1, 0), GameConfig.pipe_type_id)
	var pipe = _bm.get_building_node(Vector2i(1, 0)) as PipeNode
	assert_eq(pipe.connection_mask, 0, "孤立管道连接掩码应为 0")
	_bm.place_building(Vector2i(0, 0), GameConfig.pipe_type_id)
	var pipe_left = _bm.get_building_node(Vector2i(1, 0)) as PipeNode
