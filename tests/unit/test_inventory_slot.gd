...
const SLOT_SCENE: PackedScene = preload("res://scenes/inventory_slot.tscn")
...
func test_setup_slot_key_label() -> void:
...
func test_setup_slot_last_key_label() -> void:
...
func test_set_selected_hides_border() -> void:
	_slot.setup_slot(0, null)
	_slot.set_selected(true)
	_slot.set_selected(false)
	assert_false(_slot.find_child("SelectionBorder", true, false).visible, "set_selected(false) 应隐藏 border")

func test_setup_slot_with_type_data() -> void:
	var type_data: BuildingTypeData = BuildingTypeData.new()
