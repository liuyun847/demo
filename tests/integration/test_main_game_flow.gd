...
func test_initial_state_all_hidden():
...
func test_start_game_hides_menu_shows_bar():
...
func test_show_settings_hides_menu():
...
func test_show_start_menu_hides_settings():
...
func test_show_start_menu_pauses_game():
	EventBus.show_start_menu_requested.emit()
	await get_tree().process_frame
	assert_true(get_tree().paused, "显示开始菜单时应暂停游戏")

func test_esc_toggles_start_menu():
...
func test_slot_keys_select_inventory():
